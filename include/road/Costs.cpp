...
Costs::Costs(double af, double av, double e, double lf, double lv, double loc,
        double pc) {
...
void Costs::computeUnitRevenue(OptimiserPtr optimiser) {
    std::vector<VehiclePtr>* vehicles = optimiser->getTraffic()->getVehicles();
...
void Costs::computeEarthworkCosts() {
	...
	// represent cuts.
	unsigned int segs = this->road->getRoadSegments()->getDists()->size() - 1;
	...
	Eigen::VectorXd segLen = this->road->getRoadSegments()->getDists()->
			...
			segment(0,segs);
	Eigen::VectorXd rw = *this->road->getRoadSegments()->getWidths();
	...
	Eigen::VectorXd* cDepths = this->road->getOptimiser()->getEarthworkCosts()
			->getDepths();
	Eigen::VectorXd* cCosts = this->road->getOptimiser()->getEarthworkCosts()
			->getCutCosts();
...
void Costs::computeLocationCosts() {
    ...
    Eigen::MatrixXd* acCostsPtr = region->getAcquisitionCost();
    ...
    Eigen::MatrixXd* stabCostsPtr = region->getSoilStabilisationCost();
    ...
    std::vector<SpeciesPtr>* species = this->road->getOptimiser()
            ->getSpecies();
    ...
    if (this->road->getOptimiser()->getType() == Optimiser::SIMPLEPENALTY) {
        ...
        for (int ii = 0; ii < species->size(); ii++) {
            Eigen::MatrixXi* habMapPtr = (*species)[ii]->getHabitatMap();
            std::vector<HabitatTypePtr>* habitatTypes =
...
void Costs::computeLengthCosts() {
    ...
    Eigen::VectorXd* s = this->road->getRoadSegments()->getDists();
    Eigen::VectorXd* v = this->road->getRoadSegments()->getVelocities();
    Eigen::VectorXd* areas = cells->getAreas();
    ...
    std::vector<VehiclePtr>* vehicles = this->road->getOptimiser()
...
void Costs::computeAccidentCosts() {
    // 18 hour day
    Eigen::VectorXd* delta = this->road->getHorizontalAlignment()->getDeltas();
    Eigen::VectorXd* R = this->road->getHorizontalAlignment()->getRadii();
    Eigen::VectorXd* ssd = this->road->getVerticalAlignment()->getSSDs();
