...
/**
  ...
  */
void knn_cublas_with_indexes::printErrorMessage(cudaError_t error, int memorySize){
    printf("==================================================\n");
...
/**
  ...
  *
  */
void knn_cublas_with_indexes::knn(float* ref_host, int ref_width, float* query_host, int query_width, int height, int k, float* dist_host, int* ind_host){
    ...
    if (result){
        knn_cublas_with_indexes::printErrorMessage(result, max_nb_query_traited*size_of_float*(height+ref_width));
        return;
    }
    ...
    if (result){
        knn_cublas_with_indexes::printErrorMessage(result, ref_width * size_of_float * ( height+1 ));
        cudaFree(query_dev);
        return;
    }
	...
	if (result){
        knn_cublas_with_indexes::printErrorMessage(result, max_nb_query_traited*size_of_int*k);
        cudaFree(ref_dev);
        cudaFree(query_dev);
        return;
    }
